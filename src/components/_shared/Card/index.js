...
const Card = ({ id, imgSrc, title, overview, rating, isExpand, handleExpand, isShowSkeleton }) => {
  const { response, error, loading } = useFetchData(`/movies/${id}/casters`);

  return (
    <div className={`relative ${isShowSkeleton && "animate-pulse"}`}>
      {!isShowSkeleton && (
        ...
        <div className={`flex ${isExpand ? "flex-row" : "md:flex-col flex-row"}`}>
          ...
          <div className="flex-1">
            <div className="px-6 pt-4 pb-1">
              {isShowSkeleton ? (
                <>
                  <div className="h-4 bg-gray-100 rounded mt-1" />
                  <div className="h-4 bg-gray-100 rounded mt-1" />
                </>
              ) : (
